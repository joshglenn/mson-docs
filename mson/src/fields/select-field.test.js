...
it('should allow entries not in list of options', () => {
  const field = createField();
  field.setValue('green');
  field.set({ ensureInList: false }); // correct syntax?
  field.set({ removeIfNotInList: false });
  // ensureInList: true,
  // multiple: false,
  // removeIfNotInList: true,

  // Simulate an entry not listed in the options
  field.setValue('white');
  expect(field.getValue()).toEqual('white');
});

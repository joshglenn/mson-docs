...
// Note: We no longer instantiate a default store for the DictionaryEntryCollectionField as having a store
// introduces extra complexity that is not always needed. For example, when using the
// DictionaryEntryCollectionField without a store, you can use set() and get() to modify the underlying data
...
export default class DictionaryEntryCollectionField extends Field {
  _className = 'DictionaryEntryCollectionField';
  ...
  _getItemsPerPage() {
    if (this.get('forbidOrder')) {
      ...
      // utilize the pagination concepts in DictionaryEntryCollectionField and InfiniteLoader, but will have to
  ...
  _setParentDisableSubmit(disableSubmit) {
    ...
    // for this DictionaryEntryCollectionField, it doesn't trigger a submit on the parent form. The parent may not
  ...
  *getForms() {
  ...
  moveForm({ sourceIndex, destinationIndex, muteChange }) {
    ...
    // the store layer. This way we can support things like pagination in the DictionaryEntryCollectionField, while

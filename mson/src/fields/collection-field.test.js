...
const fillDocs = field => {
  field.addForm({
    ...
    synchronous: true // QUESTION: What does the synchronous property do?
  });
  field.addForm({
...
it('should get forms', async () => {
  const field = createField(); // create the CollectionField for testing

  fillDocs(field); // add test docs to the form

  let forms = [];
  // build an array of the docs
...
it('should set properties for nested forms', () => {
  ...
  // spy on the _setForAllForms function of the CollectionField named "field"
